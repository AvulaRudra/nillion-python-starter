...
async def main():
    ...
    if not os.path.exists(program_mir_path):
        raise FileNotFoundError(f"Program not found: {program_mir_path}")
    ...
    # Create secrets named "A", "B", and "C" with any values, ex: 6, 4, and 10
    new_secrets = nillion.NadaValues(
        {
            "A": nillion.SecretInteger(6),
            "B": nillion.SecretInteger(4),
            "C": nillion.SecretInteger(10),
        }
    )

    # Set the input parties for the secrets
    # The party names need to match the party names that are storing "A", "B", and "C" in the program
    party_name1 = "Party1"
    party_name2 = "Party2"
    party_name3 = "Party3"
